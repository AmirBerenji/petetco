...
export async function register(formdata: FormData) {
    ...
    if ((typeof register.email == 'undefined' && !register.email)
        || (typeof register.password == 'undefined' && !register.password)
        || (typeof register.password_confirmation == 'undefined' && !register.password_confirmation)
        || (typeof register.name == 'undefined' && !register.name)) {
        return { message: "Please fill all data!!!", error: true }
    }

    if (register.password != register.password_confirmation) {
        return { message: "Password and Confirn Password is not same!!!", error: true }
    }
